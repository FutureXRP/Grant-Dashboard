...
| **SF-424 auto-fill** | Opportunity page → SF-424 | Produces a **completed SF-424** (the federal cover form) for each grant: your organizational identity — legal name, EIN, UEI, address, county, congressional district, applicant type, contact + authorized representative, E.O. 12372 answer — is entered **once** and reused on every application; per-grant items (agency, Assistance Listing/CFDA, opportunity number, project title, dates, budget lines with auto-total) prefill from the opportunity and are editable. The result is a print-styled preparation copy with every numbered item answered — transcribe into Grants.gov Workspace or save as PDF for the file. Anything still blank shows an amber `[VERIFY]` flag, so nothing gets submitted half-filled. (Final entry + submission always happen in Grants.gov under your own account.) |
